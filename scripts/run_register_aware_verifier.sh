...
echo "[$(date --iso-8601=seconds)] starting post-corruption register verifier"
"$python" -u scripts/run_verifier_improvement_suite.py \
  ...
  --output-json training_logs/register_aware_verifier.json \
  --output-md training_logs/register_aware_verifier.md \
  --ranker-output checkpoints/register_aware_compat_ranker.pt \
  --target-precision 0.80 \
  --batch-size 8 \
  --ranker-epochs 1
echo "[$(date --iso-8601=seconds)] completed post-corruption register verifier"
