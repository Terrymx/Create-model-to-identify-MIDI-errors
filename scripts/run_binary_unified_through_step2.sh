...
echo "starting binary unified-correction Step 1A"
"$PYTHON" -B -u -m midi_error_detector.train \
    "${common_args[@]}" \
    --output checkpoints/transformer_keyboard_aware_binary_unified_step1a.pt \
    > training_logs/keyboard_aware_binary_unified_step1a.log \
    2> training_logs/keyboard_aware_binary_unified_step1a.err.log

echo "starting binary unified-correction scalar-surprise Step 2"
"$PYTHON" -B -u -m midi_error_detector.train \
    "${common_args[@]}" \
    --init-checkpoint checkpoints/transformer_keyboard_aware_binary_unified_step1a.pt \
    --explicit-surprise \
    --surprise-train-mask-rate 0.25 \
    --surprise-eval-groups 4 \
    --surprise-embedding-dim 16 \
    --output checkpoints/transformer_keyboard_aware_binary_unified_step2.pt \
    > training_logs/keyboard_aware_binary_unified_step2.log \
    2> training_logs/keyboard_aware_binary_unified_step2.err.log

echo "binary unified-correction Step 1A and Step 2 complete"
