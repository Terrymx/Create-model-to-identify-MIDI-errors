...
run_split() {
  local split="$1"
  "$python" -u scripts/sweep_piece_consistent_detector_thresholds.py \
    ...
    --seed 41 \
    --split "$split" \
    --output-json "training_logs/piece_consistent_detector_sweep_${split}.json"
}

echo "[$(date --iso-8601=seconds)] validation sweep"
run_split validation
echo "[$(date --iso-8601=seconds)] test sweep"
run_split test
echo "[$(date --iso-8601=seconds)] detector sweep completed"
