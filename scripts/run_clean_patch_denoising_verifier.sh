...
echo "[$(date --iso-8601=seconds)] clean-patch denoising verifier started"
"$PYTHON" -u scripts/run_clean_patch_predictor_verifier.py \
  --cache-dir training_logs/counterfactual_c_030_025_piece_cache \
  --data-root "$DATA_ROOT" \
  --output-json training_logs/clean_patch_denoising_verifier.json \
  --output-md training_logs/clean_patch_denoising_verifier.md \
  --checkpoint-dir checkpoints/clean_patch_denoising_verifier \
  ...
  --contrastive-weight 0.75 \
  --contrastive-margin 1.0
echo "[$(date --iso-8601=seconds)] clean-patch denoising verifier completed"
